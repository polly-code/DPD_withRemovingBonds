...
integer*4 sz
logical init/.true./
logical remMaxBond/.false./
logical criteriaMet/.false./
integer*4 kpbc,nbond,nangles
integer*4 arrsize
real*4 press(3)
real*4 shwi
integer*4 nstp, ind, ic, jc
integer*4 iMax, jMax, jMaxArrtype
real*4 localMaxBond(4),shareMaxBond(4), md
...
if (init) then
    ...
    allocate(mb(sz,2),rcount(nproc),displ(nproc),sndb(arrsize),rcvb(nproc*arrsize),GlobalMaxBond(nproc*4))
end if

if(mod(nstp,10000)==0) then
    remMaxBond=.true.
    localMaxBond=0
end if
...
do i=1,natms
    ...
    do k=1,nb
        ...
        if (arrtype/=0) then
            ...
            if (remMaxBond) then
                if (rijsq>localMaxBond(1) .and. abs(corr(i,1)-corr(j,arrtype))>1) then
                    localMaxBond(1)=rijsq
                    localMaxBond(2)=corr(i,1)
                    localMaxBond(3)=corr(j,arrtype)
                    localMaxBond(4)=arrtype
                end if
            end if
...
!gather info about maximum bond length
if (remMaxBond) then
    call MPI_Gather(localMaxBond,4,MPI_real4,GlobalMaxBond,4,MPI_real4,0,MPI_Comm_world,ierr)
    localMaxBond=0
end if
if (rank==0 .and. remMaxBond) then
    ind=-1
    md=0
    do i=1,4*nproc,4
        if (md<GlobalMaxBond(i)) then
            ind=i
            md=GlobalMaxBond(i)
        end if
    end do
    write(*,*) nstp,"Max_bond_length is ", md, GlobalMaxBond(ind), GlobalMaxBond(ind+1),GlobalMaxBond(ind+2),GlobalMaxBond(ind+3)
    if (md<1.5) then
        write(*,*) "Criteria maximum bond length has been met at step number ", nstp
        criteriaMet=.true.
    end if
    shareMaxBond(1)=GlobalMaxBond(ind)
    shareMaxBond(2)=GlobalMaxBond(ind+1)
    shareMaxBond(3)=GlobalMaxBond(ind+2)
    shareMaxBond(4)=GlobalMaxBond(ind+3)
    GlobalMaxBond=0
end if
call MPI_bcast(criteriaMet,1,MPI_LOGICAL,0,MPI_Comm_world,ierr)
if (criteriaMet) then
    call writerst()
	call barrier
    stop 'program finished normally'
end if
if (remMaxBond) then
    call MPI_bcast(shareMaxBond,4,mpi_real4,0,MPI_Comm_world,ierr)
    nbond=nbond-1
    ic=shareMaxBond(2)
    jc=shareMaxBond(3)
    do i = 1, cn(ic,0)
        if (cn(ic,i)==jc) then
            cn(ic,i) = cn(ic,cn(ic,0))
            cn(ic,0) = cn(ic,0) - 1
            exit
        end if
    end do
    ...
    if (rank==2) write(*,*) nstp, "Number of bonds: ", nbond
    remMaxBond=.false.
    shareMaxBond=0
end if
...
Subroutine removeMostOverstrechedBond()
! #################################################################
! #                                                               #
! #    subroutine 77:                                             #
! #    remove most overstreched bond                              #
! #                                                               #
! #################################################################
    implicit none
    save
    ...
    real*4 uni
    integer*4 reaclist(1000),nneib,reaclistc(1000),reaclistart(1000)
    real*4 distlist(1000),rij,rchem2, rjiter
    integer*4 arrtype
    integer*4 reacssize,allnum
    integer ierr
    logical init /.true./
